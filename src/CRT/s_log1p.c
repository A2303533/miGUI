...
Lp1_f = 6.6666668653e-01,	/* 3F2AAAAB */
Lp2_f = 4.0000000596e-01,	/* 3ECCCCCD */
Lp3_f = 2.8571429849e-01, /* 3E924925 */
Lp4_f = 2.2222198546e-01, /* 3E638E29 */
Lp5_f = 1.8183572590e-01, /* 3E3A3325 */
Lp6_f = 1.5313838422e-01, /* 3E1CD04F */
Lp7_f = 1.4798198640e-01; /* 3E178897 */
#pragma warning(pop)
...
__CRT_log1pf(float x)
{
	...
	if (hx < 0x3ed413d0) {			/* 1+x < sqrt(2)+  */
		if (ax >= 0x3f800000) {		/* x <= -1.0 */
			if (x == (float)-1.0) return __CRT_infinity_.__ud;// -two25_f / vzero; /* log1p(-1)=+inf */
			else return __CRT_nan_.__uf;// (x - x) / (x - x);	/* log1p(x<-1)=NaN */
		}
		if (ax < 0x38000000) {			/* |x| < 2**-15 */
			if (two25_f + x > zero			/* raise inexact */
