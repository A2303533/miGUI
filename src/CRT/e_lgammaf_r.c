...
__ieee754_lgammaf_r(float x, int *signgamp)
{
	...
	*signgamp = 1-2*((uint32_t)hx>>31);
	if(ix<0x32000000) {		/* |x|<2**-27, return -log(|x|) */
	    if(ix==0)
	        return one/vzero;
	    return -__CRT_ieee754_logf(fabsf(x));
	}
	...
	if(hx<0) {
	    *signgamp = 1;
	    ...
	    nadj = __CRT_ieee754_logf(pi/fabsf(t*x));
    ...
    /* x < 8.0 */
	    ...
	    case 3: z *= (y+2);		/* FALLTHRU */
		    r += __CRT_ieee754_logf(z); break;
	    }
    /* 8.0 <= x < 2**27 */
	} else if (ix < 0x4d000000) {
	    t = __CRT_ieee754_logf(x);
	    z = one/x;
	    y = z*z;
	    w = w0+z*(w1+y*w2);
	    r = (x-half)*(t-one)+w;
