...
__ieee754_lgamma_r(double x, int *signgamp)
{
	...
	*signgamp = 1-2*((uint32_t)hx>>31);
	if(ix<0x3c700000) {	/* |x|<2**-56, return -log(|x|) */
	    if((ix|lx)==0)
	        return one/vzero;
	    return -__CRT_ieee754_log(fabs(x));
	}
	...
	if(hx<0) {
	    *signgamp = 1;
	    ...
	    nadj = __CRT_ieee754_log(pi/fabs(t*x));
    ...
    /* x < 8.0 */
	    ...
	    case 3: z *= (y+2);		/* FALLTHRU */
		    r += __CRT_ieee754_log(z); break;
	    }
    /* 8.0 <= x < 2**56 */
	} else if (ix < 0x43700000) {
	    t = __CRT_ieee754_log(x);
	    z = one/x;
	    y = z*z;
